...
void hmvm_cuda0(matrix2<T> *mat2, T *b, int kernel, int dump_result)
{
  ...
  */
  if(kernel>=0 && kernel<4)
  {
	...
	snprintf(fname,128,"result_cuda0_%s.txt", name);
	printf("fname = %s\n", fname);
	// EXEC
...
// ######## ######## ######## ######## ######## ######## ######## ########
template void hmvm_cuda0<float>(matrix2<float>  *mat2, float *b, int kernel, int dump_result);
template void hmvm_cuda0<double>(matrix2<double> *mat2, double *b, int kernel, int dump_result);
